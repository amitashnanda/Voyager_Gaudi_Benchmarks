...
Launch (single-card):
    PT_HPU_LAZY_MODE=1 python PMPQ_evaluation_hpu.py

Launch (multi-card, 8 HPUs):
    PT_HPU_LAZY_MODE=1 python ../gaudi_spawn.py \\
        --world_size 8 --use_mpi PMPQ_evaluation_hpu.py
...
LOG_DIR = "/voyager/ceph/users/ananda2/Quantization/logs"
...
def setup_logging(rank=0):
    ...
    """
    logger = logging.getLogger("PMPQ_evaluation")
...
def quantize_model_layers(model, layer_bits_map, group_size=128, rank=0, logger=None):
    ...
    The resulting model has LinearLSQGroupWise modules with buffers
    ...
    for qualname, linear_mod, nbits in targets:
        wrapper = LinearLSQGroupWise(linear_mod, nbits, group_size=group_size)
...
def gaudi_evaluate_perplexity(model, eval_dataset, gaudi_config, logger,
    ...
    LinearLSQGroupWise modules, since HPU graph capture may fail on
...
def main():
    ...
    if rank == 0:
        ...
        logger.info(f"Trainer:   GaudiTrainer (lazy mode + HPU graphs + BF16)")
        logger.info("")
    ...
    if rank == 0:
        ...
        if not sens_dir.exists():
            logger.error("'Sensitivities' folder not found! Run Phase 1 first.")
    ...
    if rank == 0:
        ...
        logger.info("  NOTE: HPU graphs disabled for quantized model (custom LinearLSQGroupWise modules)")
    ...
    if rank == 0:
        throughput = total_tokens / eval_time_after
        ...
        logger.info(f"  Throughput:  {throughput:.0f} tokens/s")
        ...
        with open(log_path, "w") as f:
            ...
            f.write("COMPRESSION\n")
            f.write("-" * 70 + "\n")
            f.write(f"Original Size: {fp32_model_size_mb:.2f} MB\n")
            f.write(f"Quantized Size: {quant_model_size_mb_est:.2f} MB\n")
            ...
            f.write(f"throughput_tokens_per_s: {throughput:.2f}\n")
        ...
        with open(summary_log, "w") as f:
            ...
            f.write(f"FP32 PPL:    {ppl_before:.2f} ({eval_time_before:.1f}s)\n")
            f.write(f"Quant PPL:   {ppl_after:.2f} ({eval_time_after:.1f}s)\n")
            ...
            f.write(f"Throughput:  {throughput:.0f} tokens/s\n")
            f.write(f"Quality:     {quality}\n")
            if mem:
