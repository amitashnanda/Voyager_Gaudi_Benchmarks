...
1. For each layer, apply structured pruning (remove X% of weights)
...
Launch (single-card):
    PT_HPU_LAZY_MODE=1 python PMPQ_sensitivity_hpu.py --sparsity 0.3

Launch (multi-card, 8 HPUs):
    PT_HPU_LAZY_MODE=1 python ../gaudi_spawn.py \\
        --world_size 8 --use_mpi PMPQ_sensitivity_hpu.py --sparsity 0.3
...
def setup_logging(rank=0):
    ...
    """
    logger = logging.getLogger("PMPQ_sensitivity")
...
def apply_magnitude_pruning_to_layer(layer, sparsity_level):
    ...
    Runs on CPU — this is lightweight weight masking (no matrix multiply).
    The layer tensors are on CPU since model was loaded with from_pretrained()
    ...
    with torch.no_grad():
        for param in layer.parameters():
            if param.requires_grad:
                ...
                if k == 0:
                    continue
                threshold = torch.topk(torch.abs(flat_param), k, largest=True)[0][-1]
...
def main():
    ...
    if rank == 0:
        ...
        logger.info("  Clustering / I/O / logging      → CPU")
        logger.info("")
    ...
    if rank == 0:
        ...
        # Save JSON
        json_path = os.path.join("Sensitivities", f"{filename_base}.json")
        with open(json_path, "w") as f:
            json.dump(layer_sensitivities, f, indent=2)

        # Save TXT with metadata
        txt_path = os.path.join("Sensitivities", f"{filename_base}.txt")
        with open(txt_path, "w") as f:
            ...
            f.write(f"3. Evaluate perplexity with GaudiTrainer on HPU (lazy mode + BF16)\n")
