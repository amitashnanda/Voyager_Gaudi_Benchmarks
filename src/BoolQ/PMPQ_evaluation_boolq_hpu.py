"""
Phase 2: Pruning-Based Mixed-Precision Quantization (PMPQ) for TinyLlama on BoolQ
==================================================================================
...
then evaluates both baseline and quantized model accuracy on BoolQ.

Dataset: BoolQ (https://huggingface.co/datasets/google/boolq)
Task: Boolean Question Answering
Method: PMPQ (Group-Wise, Learned Step Size)
Platform: Intel Gaudi HPU (SDSC Voyager) via optimum-habana

Split Usage: VALIDATION split (3,270 samples - FULL dataset)
  Note: Uses complete validation set for accurate final metrics.
  BoolQ has no test split. Train (2,500 subset) for sensitivity, validation (full) for evaluation.
...
Pipeline:
    ...
    STEP 5: Evaluate FP32 baseline accuracy on BoolQ (HPU)
    STEP 6: Apply group-wise mixed-precision quantization (CPU)
    STEP 7: Evaluate quantized model accuracy on BoolQ (HPU)
    STEP 8-9: Report results + save
    STEP 10: Interactive text generation

Usage (single-card):
    PT_HPU_LAZY_MODE=1 python PMPQ_evaluation_boolq_hpu.py
...
def setup_logging(rank=0, log_dir=None):
    ...
    logger = logging.getLogger("PMPQ_BOOLQ_EVALUATION")
    ...
    if rank == 0:
        ...
        log_file = os.path.join(log_dir, f"boolq_evaluation_{timestamp}.log")
...
class LinearSymmetricGroupQuant(nn.Module):
    """
    Group-Wise Learned Step Size Quantization for weight quantization.
    ...
    def __init__(self, linear: nn.Linear, nbits_w: int, group_size: int = 128):
        ...
        quantized_weight = self._quantize_weight_groupwise(original_weight)
...
def quantize_model_layers(model, layer_bits_map, group_size=128, rank=0, logger=None):
    ...
    if rank == 0 and logger and quantization_stats:
        ...
        logger.info(f"  Average quantization difference: {avg_diff:.3e} (relative: {avg_rel_diff:.3%})")
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate mixed-precision quantization on BoolQ (PMPQ) - Gaudi HPU'
    )
    ...
    parser.add_argument('--batch_size', type=int, default=64,
                        help='Batch size for BoolQ evaluation inference (default: 32)')
    ...
    if rank == 0:
        logger.info("=" * 80)
        logger.info("PMPQ PHASE 2: MIXED-PRECISION QUANTIZATION EVALUATION (BoolQ)")
        logger.info("=" * 80)
    ...
    if rank == 0:
        ...
        logger.info("Computing FP32 baseline accuracy on BoolQ (HPU)...")

    acc_before, eval_time_before = evaluate_boolq_accuracy(
    ...
    if rank == 0:
        ...
        logger.info(f"  Cluster bits: {cluster_bits}")
        logger.info(f"  Layer bits map (first 5 layers): {dict(list(layer_bits_map.items())[:5])}")
    ...
    if rank == 0:
        logger.info("Evaluating quantized model accuracy on BoolQ (HPU)...")

    acc_after, eval_time_after = evaluate_boolq_accuracy(
    ...
    if rank == 0:
        ...
        logger.info("=" * 70)
        logger.info("EVALUATION RESULTS (PMPQ + GROUP-WISE) — BoolQ")
        logger.info("=" * 70)
        logger.info(f"  Model:           {model_key}")
        logger.info(f"  Benchmark:       BoolQ (accuracy)")
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_filename = f"ptq_eval_{model_key}_BoolQ_PMPQ_GroupWise_{strategy_name}_{timestamp}.txt"
        log_path = os.path.join(eval_dir, log_filename)

        with open(log_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("MIXED-PRECISION PTQ EVALUATION RESULTS (PMPQ + GROUP-WISE) — BoolQ\n")
            f.write("=" * 70 + "\n\n")
            f.write(f"Model: {model_key}\n")
            f.write(f"HuggingFace ID: {model_name}\n")
            f.write(f"Task: Question Answering (BoolQ, Yes/No)\n")
            f.write(f"Dataset: BoolQ (google/boolq), validation\n")
            ...
            f.write("COMPRESSION\n")
            f.write("-" * 70 + "\n")
            f.write(f"Original Size: {bits_to_mb(orig_bits):.2f} MB\n")
            f.write(f"Quantized Size: {bits_to_mb(quant_bits):.2f} MB\n")
