...
BoolQ evaluation with FP8 quantization using official Gaudi two-pass flow:
  MEASURE (prepare) -> forward-only calibration -> finalize_calibration
  QUANTIZE (convert) -> evaluate accuracy

Evaluates BoolQ Yes/No question answering accuracy using log-likelihood scoring.

Key differences from HellaSwag version:
- Uses BoolQ accuracy instead of HellaSwag 4-choice
- Calibration on train split (512 samples by default)
- Evaluation on validation split (~3,270 samples)
- Binary log-likelihood scoring (yes vs no)
- MAX_LENGTH: 512 (appropriate for BoolQ's shorter passages)
- Throughput measured in samples/second
...
def setup_logger(log_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("PMPQ_BOOLQ_FP8")
    ...
    fh = logging.FileHandler(log_dir / f"boolq_fp8_{ts}.log", mode="w")
    fh.setFormatter(fmt)
...
def calibration_forward_boolq(
    ...
    """
    Forward-only calibration on BoolQ train split.
    Process batch_size samples at a time, tokenize both yes/no continuations per sample, run forward passes.
...
def evaluate_boolq_accuracy(
    ...
    """
    Evaluate BoolQ accuracy using batched log-likelihood scoring on HPU.
    ...
    Args:
        ...
        max_samples: If > 0, limit evaluation to this many samples (for debugging)
    ...
    logger.info(f"[BoolQ] Evaluating {total} samples on HPU (batch_size={batch_size})...")
...
def main():
    ...
    ap.add_argument("--eval_split", type=str, default="validation", choices=["train", "validation"],
                    help="BoolQ split for evaluation (default: validation)")
    ap.add_argument("--calib_max_samples", type=int, default=512,
                    help="Max samples for calibration (default: 512)")
    ap.add_argument("--eval_max_samples", type=int, default=0,
                    help="Max samples for evaluation, 0=full split (default: 0)")
    ap.add_argument("--batch_size", type=int, default=64,
                    help="Batch size for BoolQ evaluation (default: 64, matching sensitivity)")
    ...
    ap.add_argument("--log_dir", type=str, default="logs_boolq_fp8")
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = Path(args.run_dir) if args.run_dir else Path("boolq_fp8_runs") / f"boolq_fp8_{ts}"
    ...
    logger.info(f"Loading BoolQ dataset: calib_split={args.calib_split}, eval_split={args.eval_split}")
    calib_dataset = load_boolq_dataset(args.calib_split)
    eval_dataset = load_boolq_dataset(args.eval_split)

    calib_samples = min(args.calib_max_samples, len(calib_dataset))
    eval_samples = len(eval_dataset) if args.eval_max_samples == 0 else min(args.eval_max_samples, len(eval_dataset))
    ...
    if args.run_mode in ("quantize", "both"):
        ...
        acc_baseline, t_baseline = evaluate_boolq_accuracy(
    ...
    if args.run_mode in ("measure", "both"):
        ...
        calib_time = calibration_forward_boolq(
    ...
    if acc_baseline is not None:
        ...
        logger.info(f"TinyLlama-1.1B-3T official BoolQ score: 57.83%")
